...
const App: React.FC = () => {
  ...
  const handleSaveRepair = () => {
    ...
    if (name) {
      ...
      try {
        const savedRepairs = JSON.parse(localStorage.getItem('savedRepairs') || '[]');
        savedRepairs.push(newRepair);
        localStorage.setItem('savedRepairs', JSON.stringify(savedRepairs));
        alert("¡Reparación guardada con éxito!");
      } catch (error) {
        console.error("Failed to save repairs:", error);
        alert("Hubo un error al guardar la reparación.");
      }
    }
  };
